...
class Visualiser:
    ...
    def plot_fitness_evolution(self, history: Dict[str, List], save_path: str = None):
        ...
        ax.plot(generations, mean_fitness, 'r--s', label='Mean Fitness', linewidth=2, markersize=4)
    ...
    def plot_parameter_distribution(self, best_configs: List[Dict], param_name: str,
                                     save_path: str = None):
        if save_path is None:
            save_path = os.path.join(self.output_dir, f'param_dist_{param_name}.png')

        values = [config['config'][param_name] for config in best_configs
    ...
    def plot_multi_objective_tradeoff(self, best_configs: List[Dict], save_path: str = None):
        if save_path is None:
        ...
        scatter1 = ax1.scatter(accuracies, stabilities, c=range(len(accuracies)),
        ...
        scatter2 = ax2.scatter(accuracies, runtimes, c=range(len(accuracies)),
                               cmap='viridis', s=100, alpha=0.6)
    ...
    def generate_all_plots(self, result: Dict[str, Any]):
        ...
        if 'history' in result:
            ...
            if 'best_configs' in history:
                ...
                param_names = ['learning_rate', 'batch_size', 'dropout_rate',
                               'num_layers', 'optimiser']
                for param_name in param_names:
                    plot_path = self.plot_parameter_distribution(best_configs, param_name)
                    if plot_path:
                        plots[f'param_{param_name}'] = plot_path

                plots['multi_objective'] = self.plot_multi_objective_tradeoff(best_configs)
